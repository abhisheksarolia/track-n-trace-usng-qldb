...
def verify_coldchain(driver, ledger_name, package, itemid):
    ...
    statement = "SELECT r.metadata.id As id, r.blockAddress AS blockAddress FROM _ql_committed_Item AS r WHERE r.data.ItemId = '{}' and r.data.PackageLabel = '{}'".format(itemid, package)
...
def verify_batch_compliance(driver, ledger_name, batch, itemid):
    ...
    statement = "SELECT r.metadata.id As id, r.blockAddress AS blockAddress FROM _ql_committed_Item AS r BY r_id WHERE r.data.ItemId = '{}' and r.data.ItemSpecifications.MfgBatchNumber = '{}'".format(itemid, batch)
